...
ggplot(aes(x = chlorides), data = whitewines) +
  ...
  scale_x_continuous(limits = c(0, quantile(whitewines$chlorides, .99)), 
                     breaks = seq(0,quantile(whitewines$chlorides, .99), 0.05)) + 
  ggtitle("Chlorides Distribution")

# exclude more outliers
ggplot(aes(x = chlorides), data = whitewines) +
  geom_histogram(binwidth = 0.005, color = 'black', fill = 'blue') + 
  scale_x_continuous(limits = c(0, quantile(whitewines$chlorides, .98)), 
                     breaks = seq(0,quantile(whitewines$chlorides, .98), 0.05)) + 
...
Transform scale to log 10 and square root for residual sugar. In log10, I saw bimodal distribution.
...
p6 <- ggplot(aes(x = free.sulfur.dioxide), data = whitewines) +
        ...
        scale_x_continuous(limits = c(0, quantile(whitewines$ free.sulfur.dioxide, .997)), 
                           breaks = seq(0, quantile(whitewines$ free.sulfur.dioxide, .998), 10)) + 
...
ggplot(aes(x = alcohol, y = quality),data = whitewines) +
  geom_point(alpha = 1/8, fill = I('#F79420'), color = I('blue'), position = 'jitter', shape = 22) +
...
Higher quality wines tend to have higher alcohol content. Coefficient between quality and alcohol content is 0.436.
...
Quality decreases as chlorides increases, especially for wines with quality between 5 and 7.  Coefficient between quality and chlorides is -0.21.

```{r echo=FALSE, Bivariate_Plots_04}
ggplot(aes(x = density  , y = quality),data = whitewines) +
  geom_point(alpha = 1/10, fill = I('#F79420'), color = I('blue'), position = 'jitter', shape = 22) +
  scale_x_continuous(limits = c(0.9870, quantile(whitewines$density, .99)), 
                     breaks = seq(0.9870, quantile(whitewines$density, .99), 0.002))  +
  scale_y_continuous(limits = c(3, 9), breaks = seq(3, 9, 1)) +
  ggtitle("Quality Vs Density") 

```

Quality decreases when density increases. Coefficient between qualtiy and density is -0.307
...
```
I can't see much pH impact on quality of wines(excluding quality = 3 and 9). So I would like to draw a histogram by quality to cross exam
...
ggplot(aes(x = quality.factor , y = residual.sugar),data = whitewines_sub) +
  geom_boxplot() +  
  xlab("quality") +
  ylim(0, quantile(whitewines$residual.sugar, .99)) +
  coord_flip() +
  ggtitle("Quality Vs Residual Sugar") 

ggplot(aes(x = residual.sugar), data = whitewines_sub ) +
  geom_histogram(binwidth = 0.5, color = 'black', fill = 'blue') + 
  scale_x_continuous(limits = c(0, quantile(whitewines$residual.sugar, .999)), 
                     breaks = seq(0, quantile(whitewines$residual.sugar, .999), 5)) +   
  ggtitle("Residual Suguar Distribution by quality") + 
  facet_wrap(~quality, scales = "free_y", ncol = 1)
```

To my surprise, quality 4 wines has the lowest median residual sugar even lower than quality 8 wines. From quality between 5-7, the quality increase as residual sugar decreases.  However, quality 8 wines has higher median residual sugar than quality 7 wines. So the correction between quality and residual sugar doesn't look not very strong and their coefficient is only 0.0976
...
Except wines with quality 4. It looks like that higher quality wine tends to have lower total free sulfur dioxide.However, coefficient between quality and total SO2 is only 0.0537


```{r echo=FALSE, Bivariate_Plots_09}
ggplot(aes(x = volatile.acidity, y = quality),data = whitewines) +
  geom_point(alpha = 1/4, fill = I('#F79420'), color = I('blue'), position = 'jitter', shape = 22) +
 scale_x_continuous(limits = c(0, quantile(whitewines$volatile.acidity, .99)), 
...
To my surprise, quality 6, 7, 8 has very close median volatile acidity. It is not very obvious that volatile acidity has much impact on wine quality. Coefficient between quality and volatile acidity is -0.195
...
Density decreases when alcohol increases. Density increase when residual sugar increases. And although there are some spikes, overall, residual sugar decrease as alcohol increase. Here are their coefficients
...
p24 <- ggplot(aes(x = total.sulfur.dioxide  , y = free.sulfur.dioxide),data = whitewines) +
  geom_point(alpha = 1/10, fill = I('#F79420'), color = I('black'),  shape = 22) +
 scale_x_continuous(limits = c(0, quantile(whitewines$total.sulfur.dioxide, .99)), 
                    breaks = seq(0, quantile(whitewines$total.sulfur.dioxide, .99), 20))  +
...
Free sulfur dioxide has strong positive correlation with total sulfur dioxide. On the other hand, Volatile acidity looks irrelevant to fixed acidity. Here are their Coefficients
...
ggplot(aes(x = total.acidity, y = pH, color = quality.factor),data = whitewines_sub, ) +
  ...
  scale_color_brewer(type = 'div',
    guide = guide_legend(title = 'Quality', reverse = T,
    override.aes = list(alpha = 1, size = 2))) + 
  scale_x_continuous(limits = c(4.5, quantile(whitewines$fixed.acidity, .99)), 
...
ggplot(aes(x = alcohol, y = total.sulfur.dioxide, color = quality.factor),data = whitewines, ) +
  ...
  scale_color_brewer(type = 'qual', palette=3,
                     guide = guide_legend(title = 'Quality', reverse = T, override.aes = list(alpha = 1, size = 2))) + 
 scale_x_continuous(limits = c(8.0, 14.20), breaks = seq(8.0, 14.20, 2)) +
